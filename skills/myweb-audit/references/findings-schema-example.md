...
repair_target:
  ...
  primary_owner: web/frontend/src/composables/useMarketFilters.ts
...
- `repair_target.primary_owner` should identify the smallest stable owner that should be edited first.
...
- If multiple files are listed in `repair_target.files`, the `dedupe_key` should still use the primary owner rather than a downstream wrapper.
