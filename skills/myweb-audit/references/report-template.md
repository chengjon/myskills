...
- Why consolidated:
- Primary owner:
- Fix bucket: [fix-now | fix-with-shared-impact-review | defer]
...
- Manifest resume cursor after approval:
...
- Risk notes:
- GitNexus staged verdict origin:
...
- Record the chosen `primary owner` for each consolidated issue whenever the repair target spans multiple files.
- Use repair-package language in summaries so the user can see what was approved, deferred, or held for shared-impact review.
- When a formal approval package exists, record its artifact path in both `Repair Plan` and `Approval Accounting`.
...
- If GitNexus staged detection was not isolated to the intended batch files, report it under `Mixed staged observations` rather than presenting it as the batch verdict.
- When structured artifacts exist, `Artifact validation commands run` should prefer the manifest-truth path before listing lower-level schema checks.
