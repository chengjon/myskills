...
- [ ] Repair approval package is recorded when the batch used a formal approval artifact.
- [ ] Deferred items include severity, reason, and dependency.

## Fix Accounting
- [ ] User approval for repaired findings was recorded.
- [ ] Approval package path or inline approval package record is stated when applicable.
...
- If GitNexus staged detection was based on a mixed staged set, record it as an observation rather than the current batch's isolated risk verdict.
