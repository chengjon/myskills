...
算法:
  - 多级匹配: exact(价格在K线范围内) → boundary(<1%误差) → failed
...
def fetch_15min_kline(code: str, count: int = 5000, force_refresh: bool = False) -> List[Dict]:
    """
    获取15分钟K线数据(带MySQL缓存)
    ...
    if inserted:
        print(f"  [{code}] 缓存写入{inserted}条", end=" ", flush=True)
