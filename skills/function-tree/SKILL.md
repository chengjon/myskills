...
| `/ft:new-node-batch <program> --from-dirs <dir>` | `new-node-batch <program> --from-dirs <dir> [--id-prefix <text>] [--pattern <glob>] [--parent <id>] [--track <t>] [--mainline-id <id>] [--depth <n>] [--type <kind>] [--dry-run]` | Walk `<dir>` one level deep and create one node per subdir; node id = `<id-prefix><subdir>`, title = subdir, ref = `<dir>/<subdir>`. Useful for bootstrapping a module-shaped subtree. |
| `/ft:reparent <program> <node-id> --parent <id>` | `reparent <program> <node-id> --parent <id> [--mainline-id <id>] [--depth <n>] [--track <t>]` | Atomically reparent an existing node without hand-editing `nodes.json`; use to fix parallel-mainline or depth-mismatch violations surfaced by `validate full`. |
| `/ft:suggest-nodes <program> [--yes]` | `suggest-nodes <program> [--yes] [--dry-run]` | Print a reviewable draft of nodes for every auto-discovered candidate (README features, source modules, UI/API routes, etc.). `--yes` batch-imports them. Default is dry-run so the user stays in the loop. |
| `/ft:promote-pkgs <program> [--yes]` | `promote-pkgs <program> [--yes] [--dry-run]` | Promote pkg-root first-level subpackages (盲区 A) into planning nodes. Works on `<pkg>/__init__.py`, `package.json`, `Cargo.toml`, `go.mod` layouts. |
| `/ft:promote-readme <program> [--yes]` | `promote-readme <program> [--yes] [--dry-run]` | Promote README H2/H3 headings (盲区 B) into mainline planning nodes. Anchor links are resolved to file/dir evidence when possible. |
| `/ft:promote-entrypoints <program> [--yes]` | `promote-entrypoints <program> [--yes] [--dry-run]` | Promote manifest entry-points (盲区 C) — `[project.scripts]`, `[project.entry-points.*]`, `package.json bin/exports`, `Cargo.toml [[bin]]`. Double-evidenced ones (manifest + target file defines the symbol) land on mainline. |
| `/ft:promote-untracked <program> [--yes]` | `promote-untracked <program> [--yes] [--dry-run]` | Promote git worktree `??`/`A` files (盲区 D) into backlog `待实现` nodes — the strongest in-progress work signal. `.governance/` / `.claude/` / `node_modules/` are auto-skipped. |
| `/ft:promote-changelog <program> [--yes]` | `promote-changelog <program> [--yes] [--dry-run]` | Promote CHANGELOG release bullets (E5) into mainline `声明实现` nodes. Recognizes Markdown (`## x.y.z`) and RST (`Version x.y.z` + underline) formats across CHANGELOG/CHANGES/HISTORY/RELEASES/NEWS. |
