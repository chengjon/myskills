...
function writeFunctionTreeDoc(root, context) {
  ...
  const content = renderFunctionTreeDoc(root, context, notes);
...
function renderFunctionTreeDoc(root, context, notes) {
  ...
  return [
    ...
    `- Project: ${info.name}`,
    `- Root: \`${root}\``,
    `- Current HEAD: \`${info.head || 'unknown'}\``,
    `- Manifests: ${formatList(info.manifests)}`,
    `- Documentation: ${formatList(info.docs)}`,
    `- Source roots: ${formatList(info.sourceRoots)}`,
    '',
    '## Skill Activation',
    '',
    'Use the `function-tree` skill when work mentions FUNCTION_TREE, function tree, `/ft`, governance gate, active gate, scope authorization, evidence collection, task card, or closeout.',
    '',
    'Explicit request examples:',
    '',
    '- `Use the function-tree skill to plan this change.`',
    '- `/ft:init <program> --ref <function-tree-node>`',
    '- `/ft:observe <program> <node-id> --evidence <path-or-note>`',
    '- `/ft:authorize <program> <node-id>`',
    '- `/ft:transition <program> <node-id> --to <status>`',
    '- `/ft:closeout <program> <node-id>`',
    '',
    '## Governance Programs',
    '',
    '| Program | FUNCTION_TREE ref | Description | Nodes | Active gates | Tree |',
    '|---------|-------------------|-------------|-------|--------------|------|',
    ...programRows.map((row) => `| ${row.map(escapeCell).join(' | ')} |`),
    '',
    '## Operating Loop',
    '',
    '1. Initialize a program with `init`.',
    '2. Create a node with `new-node`.',
    '3. Record baseline evidence with `observe` before editing source files.',
    '4. Prepare allowed paths, non-goals, and gates with `authorize`.',
    '5. Move through legal states with `transition`.',
    '6. Keep edits inside active authorization with `scope-check` or `install-guard`.',
    '7. Record landed evidence with `closeout`.',
    '',
    '## State Files',
    '',
    '- `.governance/active-gates.json` is the machine-owned active gate source.',
    '- `.governance/active-gates.md` is the generated human-readable active gate summary.',
    '- `.governance/programs/<program>/nodes.json` is the program node state source.',
    '- `.governance/programs/<program>/cards/*.yaml` stores authorization task cards.',
    '- `.governance/backups/` stores previous `FUNCTION_TREE.md` copies before updates.',
