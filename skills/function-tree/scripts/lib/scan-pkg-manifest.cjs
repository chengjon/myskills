...
function collectWorktreeCandidates(root) {
  ...
  for (const line of lines) {
    ...
    if (!file) continue;
    // Skip generated governance artifacts and third-party noise. FUNCTION_TREE.md
    // is intentionally generated by this skill, so treating it as an untracked
    // planned feature makes `ft doc` non-idempotent immediately after `ft init`.
    if (/^(?:\.governance|\.claude|node_modules)\//.test(file)) continue;
    if (file === 'FUNCTION_TREE.md') continue;
    if (status === '??') {
      candidates.push({
...
function collectVerificationGateCandidates(root) {
  ...
  if (fs.existsSync(workflowsDir)) {
    ...
    for (const f of files) {
      ...
      // FT_REVIEW P2 re-baseline: extend metadata skip list so `ci-job:` emission
      // captures real jobs and not structural keys. The skip list merges the
      // pre-existing keys (name/on/jobs/env/runs-on/steps/permissions/concurrency)
      // with the additional structural keys surfaced by the candidate-classification
      // review: branches/paths-ignore/needs/with/matrix.os/timeout-minutes plus the
      // step-level `uses`/`if` (which live under `steps:` and so are already out of
      // this regex's scope — listed here as documentation of what we deliberately
      // ignore at the step scope, not at the top-level job scope).
      //
      // matrix-strategy handling: `strategy:` and `matrix:` are structural keys
      // (skipped); the actual matrix *expansions* (`matrix.os`, `matrix.version`)
      // are not emitted as separate gates — they are configuration of the parent
      // job. If a workflow wants a per-matrix gate, it should be expressed as a
      // separate job id.
      const SKIP_KEYS = new Set([
        'name', 'on', 'jobs', 'env', 'runs-on', 'steps', 'permissions', 'concurrency',
        'branches', 'paths-ignore', 'paths', 'needs', 'with', 'matrix', 'matrix.os',
        'matrix.version', 'matrix.node-version', 'timeout-minutes', 'strategy',
        'continue-on-error', 'if', 'uses', 'run', 'working-directory', 'shell',
        'env', 'defaults', 'services', 'container', 'environment', 'outputs',
      ]);
      const jobRe = /^\s{2,}([A-Za-z0-9_.-]+)\s*:/gm;
      let m;
      while ((m = jobRe.exec(text)) !== null) {
        if (SKIP_KEYS.has(m[1])) continue;
        // Skip keys that look like matrix.foo.bar entirely — they are matrix
        // expansion parameters, not jobs.
        if (/^matrix\./.test(m[1])) continue;
